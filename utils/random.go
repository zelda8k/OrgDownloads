...
func OrgRandom() {
	ImagensPath := filepath.Join(os.Getenv("USERPROFILE"), "Downloads", "Random")
	...
	for _, file := range files {
		...
		} else if strings.Contains(file.Name(), ".tmp") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".bak") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".ini") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".cfg") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
