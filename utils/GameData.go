...
func OrgGameData() {
	ImagensPath := filepath.Join(os.Getenv("USERPROFILE"), "Downloads", "GameData")
	...
	for _, file := range files {
		...
		if strings.Contains(file.Name(), ".sav") {
		...
		} else if strings.Contains(file.Name(), ".dat") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".pak") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".rom") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
