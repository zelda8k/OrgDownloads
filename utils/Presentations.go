...
func OrgPresentations() {
	ImagensPath := filepath.Join(os.Getenv("USERPROFILE"), "Downloads", "Presentations")
	...
	for _, file := range files {
		...
		if strings.Contains(file.Name(), ".ppt") {
		...
		} else if strings.Contains(file.Name(), ".pptx") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".odp") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
