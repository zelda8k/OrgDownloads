...
func OrgCompressed() {
	ImagensPath := filepath.Join(os.Getenv("USERPROFILE"), "Downloads", "Compressed")
	...
	for _, file := range files {
		...
		} else if strings.Contains(file.Name(), ".rar") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".7z") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".gz") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
		...
		} else if strings.Contains(file.Name(), ".bz2") {
			originPath := filepath.Join(downloads, file.Name())
			destinationPath := filepath.Join(ImagensPath, file.Name())
